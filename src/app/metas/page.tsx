...
import { Plus, Edit, Trash2, MoreHorizontal, CircleAlert, Target, Home, Car, GraduationCap, Plane, PiggyBank, Award, Calendar as CalendarIcon, DollarSign, TrendingUp, TrendingDown, CheckCircle, Clock } from "lucide-react"
...
export default function MetasPage() {
  ...
  const budgetAmountFormat = useCurrencyFormat(formData.budgetAmount.toString())
  const currentAmountFormat = useCurrencyFormat(formData.currentAmount.toString())
  ...
  const iconOptions = [
    ...
    { name: 'Calendário', value: 'CalendarIcon' },
  ...
  const handleSubmit = () => {
    ...
    const budgetAmountValue = budgetAmountFormat.numericValue
    const currentAmountValue = currentAmountFormat.numericValue

    if (budgetAmountValue <= 0) {
      toast.error("O valor orçado deve ser maior que zero")
      return
    }
    ...
    if (editingGoal) {
      ...
      setGoals(goals.map(goal => 
        goal.id === editingGoal.id 
          ? { ...goal, ...formData, budgetAmount: budgetAmountValue, currentAmount: currentAmountValue }
          : goal
      ))
  ...
  const handleEdit = (goal: Goal) => {
    ...
    budgetAmountFormat.setValue(goal.budgetAmount.toString())
    currentAmountFormat.setValue(goal.currentAmount.toString())
  ...
  const resetForm = () => {
    ...
    budgetAmountFormat.setValue('')
    currentAmountFormat.setValue('')
    setEditingGoal(null)
  ...
  const getProgressPercentage = (goal: Goal) => {
    return goal.budgetAmount > 0 ? (goal.currentAmount / goal.budgetAmount) * 100 : 0
  }
  ...
  const renderGoalCard = (goal: Goal) => {
    ...
    const remaining = goal.budgetAmount - goal.currentAmount
            ...
            <div className="mt-2">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm">
                  {formatCurrency(goal.currentAmount)} de {formatCurrency(goal.budgetAmount)}
                </span>
  ...
  // Calculate totals - Fixed variable name
  const totalBudgetAmount = goals.reduce((sum, goal) => sum + goal.budgetAmount, 0)
  const totalCurrentAmount = goals.reduce((sum, goal) => sum + goal.currentAmount, 0)
  const totalProgress = totalBudgetAmount > 0 ? (totalCurrentAmount / totalBudgetAmount) * 100 : 0
              ...
              <div className="px-4 lg:px-6">
                <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-4 @5xl/main:grid-cols-4">
                  <Card className="@container/card">
                    ...
                    <CardFooter className="flex-col items-start gap-1.5 text-sm">
                      <div className="line-clamp-1 flex gap-2 font-medium">
                        Metas cadastradas <TrendingUp className="size-4" />
                      </div>
                  ...
                  <Card className="@container/card">
                    ...
                    <CardFooter className="flex-col items-start gap-1.5 text-sm">
                      <div className="line-clamp-1 flex gap-2 font-medium">
                        Meta consolidada <TrendingUp className="size-4" />
                      </div>
                      <div className="text-muted-foreground">
                  ...
                  <Card className="@container/card">
                    ...
                    <CardFooter className="flex-col items-start gap-1.5 text-sm">
                      <div className="line-clamp-1 flex gap-2 font-medium">
                        Progresso acumulado <TrendingUp className="size-4" />
                      </div>
                      <div className="text-muted-foreground">
                        {totalProgress.toFixed(1)}% do total alcançado
                      </div>
                    </CardFooter>
                  </Card>
                  <Card className="@container/card">
                    <CardHeader>
                      ...
                      <CardTitle className="text-2xl tabular-nums @[250px]/card:text-3xl text-red-600">
                        {formatCurrency(Math.max(0, totalBudgetAmount - totalCurrentAmount))}
                      </CardTitle>
                      <CardAction>
                        <Badge variant="outline">
                    ...
                    <CardFooter className="flex-col items-start gap-1.5 text-sm">
                      <div className="line-clamp-1 flex gap-2 font-medium">
                        Valor restante <TrendingDown className="size-4" />
                      </div>
                      <div className="text-muted-foreground">
                        Falta para atingir as metas
                      </div>
                    </CardFooter>
                  </Card>
                </div>
          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="*:not-first:mt-2">
                  <Label htmlFor="budgetAmount">Valor Orçamento</Label>
                  <Input
                    id="budgetAmount"
                    value={budgetAmountFormat.formattedValue}
                    onChange={budgetAmountFormat.handleChange}
                    placeholder="R$ 0,00"
                  />
                </div>

                <div className="*:not-first:mt-2">
                  <Label htmlFor="currentAmount">Valor Orçamento</Label>
                  <Input
                    id="currentAmount"
                    value={currentAmountFormat.formattedValue}
                    onChange={currentAmountFormat.handleChange}
                    placeholder="R$ 0,00"
                  />
                </div>
              </div>
